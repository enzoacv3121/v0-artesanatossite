  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-gray-800 font-sans">
      ...
      <main className="flex-grow w-full max-w-[1280px] mx-auto px-4 sm:px-6 py-8 lg:py-12">
        ...
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-16 mb-20 items-start">
          ...
          <div className="lg:col-span-7 flex flex-col-reverse lg:flex-row gap-4">
            <div className="flex lg:flex-col gap-3 overflow-x-auto lg:overflow-visible hide-scrollbar py-2 lg:py-0">
              {[1, 2, 3].map((i) => (
                /* MUDANÇA: rounded-xl overflow-hidden */
                <div key={i} className="w-16 h-16 lg:w-[70px] lg:h-[90px] flex-shrink-0 cursor-pointer border border-transparent hover:border-gray-900 transition-all opacity-70 hover:opacity-100 rounded-xl overflow-hidden">
                  <img src={p.imagem_url || '/placeholder.jpg'} alt={`Thumb ${i}`} className="w-full h-full object-cover" />
                </div>
              ))}
            </div>
            {/* MUDANÇA: rounded-2xl overflow-hidden shadow-sm */}
            <div className="flex-1 relative bg-stone-50 aspect-[4/5] lg:aspect-[3/4] rounded-2xl overflow-hidden shadow-sm">
               {/* MUDANÇA: rounded-bl-xl */}
               <span className="absolute top-0 right-0 bg-[#25D366] text-white text-[10px] font-bold px-3 py-1.5 rounded-bl-xl uppercase tracking-widest z-10 shadow-sm">
                 Compre 4 Pague 3
               </span>
              <img 
          ...
          <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-24">
            ...
            <div className="py-2">
              <div className="flex items-baseline gap-3">
                ...
                <span className="text-3xl font-bold text-black tracking-tighter">R${p.preco.toFixed(2).replace('.', ',')}</span>
              </div>
              <p className="text-xs text-gray-500 mt-1">
                3x de R${(p.preco / 3).toFixed(2).replace('.', ',')} sem juros
              </p>
              <button className="text-xs underline text-gray-800 mt-2 flex items-center gap-1 hover:text-black transition-colors">
                 <CreditCard size={12}/> Ver formas de pagamento
              </button>
            </div>

            {/* MUDANÇA: bg-green-50/50, border-green-200, rounded-xl */}
            <div className="border border-green-200 p-5 bg-green-50/50 rounded-xl">
              <p className="text-green-700 font-bold text-sm mb-1.5 uppercase tracking-wide">Compre 4 e pague 3!</p>
              <p className="text-stone-600 text-xs leading-relaxed">Você pode aproveitar esta promoção em qualquer produto da loja.</p>
            </div>

            <ProductInteractive produtoId={p.id_produtos} estoque={p.estoque} />

            <div className="border-t border-stone-100 pt-6 space-y-3 mt-4">
        ...
        <div className="border-t border-stone-100 pt-16 mt-12 max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-12 gap-12 text-stone-600 text-sm leading-8">
            ...
            <div className="md:col-span-5">
                ...
                <div className="bg-stone-50 p-8 rounded-2xl border border-stone-100">
                    ...
                    <p className="mt-6 text-xs text-stone-400 italic leading-relaxed">
